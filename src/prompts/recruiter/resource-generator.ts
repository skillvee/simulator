...
- **Engineering roles** (backend, frontend, fullstack, DevOps, SRE, platform engineers): You MUST generate at least one \`repository\` resource with a Quick Start section containing shell commands in a fenced code block AND code snippets for the specific files/functions the task asks the candidate to work on. If the task says "refactor the useComments hook" or "fix the bid handler", the repository resource MUST include that hook's or handler's current implementation code — not just an architecture overview. The candidate cannot browse the codebase; the only code they see is what you put in the resource.
